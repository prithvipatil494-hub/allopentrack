...
export default function LocationTracker() {
  ...
  useEffect(() => {
    const storedId = localStorage.getItem('myTrackId');
    const trackId = storedId || generateTrackId();
    if (!storedId) {
      localStorage.setItem('myTrackId', trackId);
    }
  ...
  const fetchFriendLocation = async () => {
    ...
    try {
      const result = await window.storage.get(`location:${friendTrackId}`, true);
        ...
        if (Date.now() - data.timestamp < 30000) {
          ...
          if (mapInstanceRef.current && window.L) {
            ...
            if (friendPolylineRef.current) {
              const currentPath = friendPolylineRef.current.getLatLngs();
              currentPath.push([data.lat, data.lng]);
              friendPolylineRef.current.setLatLngs(currentPath);
            } else {
              friendPolylineRef.current = window.L.polyline([[data.lat, data.lng]], {
                color: '#22c55e',
                weight: 5,
                opacity: 0.9,
                smoothFactor: 1
